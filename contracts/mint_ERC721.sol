...
contract BasicMinter is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => string) private _tokenURIs; // Store token-specific URIs

    // Events
    event Mint(address indexed minter, uint256 tokenId, string tokenURI);

    constructor() ERC721("ArbitrumNFT", "ARBFT") Ownable(msg.sender) {} // Pass msg.sender to Ownable constructor
    ...
    function mint(uint256 quantity, string memory _tokenURI) public payable {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            _setTokenURI(tokenId, _tokenURI); // Set the token-specific URI
